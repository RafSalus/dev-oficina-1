...
import { X, SignOut, CaretRight, SquaresFour } from '@phosphor-icons/react'

export function MobilePortalMenuSheet({ isOpen, onClose, title, subtitle, menuCategories, iconsMap, onSignOut }) {
  ...
  const handleNavigate = (path, isExternal) => {
    onClose()
    if (isExternal) {
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      >
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-[#f2f4f7] shrink-0">
          <div>
            <h2 className="text-sm font-extrabold text-[#101828] tracking-tight">{title}</h2>
            {subtitle && <p className="text-[11px] text-[#667085] font-medium">{subtitle}</p>}
          </div>
          <button
            type="button"
            onClick={onClose}
            ...
            <div key={category.id}>
              ...
              <div className="space-y-0.5">
                {category.items.map((item) => {
                  const IconComponent = iconsMap[item.icon] || SquaresFour
                  return (
                    <button
                      key={item.id}
                      type="button"
                    ...
                    >
                      ...
                      <span className="flex-1 text-xs font-semibold text-[#344054] truncate">{item.label}</span>
