...
export function MobilePortalHeader({
  ...
  settingsPath,
  onSignOut,
  notificationText = 'Sistema operacional ativo',
  notificationSubtext = 'Banco de dados e serviços conectados com sucesso.',
}) {
      ...
      <div className="h-14 px-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            ...
            {brandSubtitle && (
              <span className="text-[9px] font-semibold tracking-wider uppercase text-[#667085] truncate">
                {brandSubtitle}
              </span>
            )}
          </div>
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-[#e4e7ec] py-3 z-50 animate-in fade-in zoom-in-95 duration-150">
                ...
                <div className="px-4 py-2.5 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-[#101828] leading-snug">{notificationText}</p>
                    <p className="text-[11px] text-[#667085] mt-0.5">{notificationSubtext}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {showProfile && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-[#e4e7ec] py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="px-4 py-2 border-b border-[#f2f4f7]">
                  <p className="text-xs font-bold text-[#101828]">{profileName}</p>
                  {profileSubtitle && <p className="text-[11px] text-[#667085] truncate">{profileSubtitle}</p>}
                </div>
                <div className="py-1">
                  {settingsPath && (
                    <button
